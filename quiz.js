window.onload = () => {
  $(function() {
    ...
    function alertMessage(message, type, duration) {
      ...
      if (type == "success") {
        alert.innerHTML += "<i class='fas fa-check ml-2'></i>";
        $(alert).css({ color: "green" });
      }
      if (type == "danger") {
        alert.innerHTML += "<i class='fas fa-times ml-2'></i>";
        $(alert).css({ color: "red" });
      }
      // Animations
      $(alert).animate({ left: "50px" }, duration, function() {
        setTimeout(function() {
          $(alert).animate({ left: "-150px" }, duration);
        }, 1500);
      });
    }
        ...
        if (this.readyState == 4 && this.status == 200) {
          ...
          function submitAnswer() {
            if (count < 9) {
              ...
              if (correct) {
                marks++;
                alertMessage("Correct", "success", 500);
              } else {
                alertMessage("Incorrect", "danger", 500);
              }
